...
def process_time(df):
    ...
    df["NumberofLanes"] * df["is_peak_hour"]
    )
...
X["time_block"] = X["time_block"].astype(str)
X_test["time_block"] = X_test["time_block"].astype(str)

X["weather_peak"] = X["weather_peak"].astype(str)
X_test["weather_peak"] = X_test["weather_peak"].astype(str)
